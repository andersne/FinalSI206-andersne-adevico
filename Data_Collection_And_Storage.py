...
def get_populations(state):
    ...
    for websiteLink in websiteLinks:
        ...
        try:
            ...
            print("Unfortunately, this state is missing information on certain counties and therefore we are unable to continue the program. Re-run the program and choose a different state.")
            sys.exit()
...
def main():
    ...
    time_for_state2 = len(cur.fetchall())
    counties_state1 = len(get_countyNames(state1))

    if time_for_state2 < counties_state1:
        ...
        cur.execute(f'SELECT countyID FROM {state1}CovidInfo')
        time_to_write = len(cur.fetchall())
        if time_to_write == len(get_countyNames(state1)):
    ...
    if(time_for_state2 == counties_state1):
        ...
        time_to_write = len(cur.fetchall())
        if time_to_write == len(get_countyNames(state2)):
